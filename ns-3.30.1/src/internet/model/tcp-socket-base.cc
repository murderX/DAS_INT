...
//                   MakeEnumChecker (CLOSED, "Closed"))
    ...
    .AddAttribute ("UseEDF", "Determine if EDF is enabled",
                   BooleanValue(false),
                   MakeBooleanAccessor(&TcpSocketBase::m_use_edf),
                   MakeBooleanChecker()
                  )
    .AddAttribute ("EnableINT", "Determine if INT is enabled",
                   BooleanValue(false),
                   MakeBooleanAccessor(&TcpSocketBase::m_int_das),
                   MakeBooleanChecker()
                  )
    .AddAttribute ("INT_CSMode", "Determine the mode of the socket",
                   BooleanValue(true),
                   MakeBooleanAccessor(&TcpSocketBase::m_das_mode),
                   MakeBooleanChecker()
                  )
    .AddAttribute ("Deadline", "The deadline for the application",
                   UintegerValue(50000),
                   MakeUintegerAccessor(&TcpSocketBase::m_deadline),
                   MakeUintegerChecker<uint32_t>()
                  )
    .AddAttribute ("Alpha", "Rate to adjust delay prediction",
                   DoubleValue(1.0),
      ...
      if (m_ecnMode == EcnMode_t::ClassicEcn)
      ...
      else
        {
          if(m_int_das) {
            if(m_das_mode == 0)
                m_c_header.mode = 1;
                m_c_header.totalHop = 0;
                m_c_header.deadline = m_deadline;
...
void 
TcpSocketBase::ProcessINT(TcpHeader tcpHeader, Address fromAddress, Address toAddres, Ptr<Packet>  p) {
  IntHeader itHeader;
  if(tcpHeader.HasOption(TcpOption::INT)) {
    TcpHeader temp;
    p->RemoveHeader(temp);
    Ptr<const IntTcpOption> intOption = DynamicCast<const IntTcpOption>(tcpHeader.GetOption(TcpOption::INT));
    p->RemoveHeader(itHeader);
    if(m_das_mode == 0) {
      // Client side
      // Updating INT info
      // std::cout<<"Client Received INT header:";
      // itHeader.Print(std::cout);
      for(int i=0; i < itHeader.maxHop; i++) {
        if(m_c_header.mode == 1) {
           m_c_header.hop[i] = itHeader.hop[i];
        }
        else {
          m_c_header.hop[i].setDelay((uint32_t)((1 - m_alpha) * m_c_header.hop[i].GetDelay() + m_alpha * itHeader.hop[i].GetDelay()));
          m_c_header.hop[i].setUrgency((uint32_t)((1 - m_alpha) * m_c_header.hop[i].GetUrgency() + m_alpha * itHeader.hop[i].GetUrgency()));
        }
      }
      m_c_header.mode = 2;
      m_c_header.maxHop = itHeader.maxHop;
      m_c_header.totalHop = itHeader.totalHop;
      m_c_header.counter = 0;
    }
    else if(m_das_mode == 1) {
      // Server side
      // Storing/Updating INT info
      // m_peer[fromAddress] = itHeader;
      if(m_peer.find(fromAddress) == m_peer.end()) {
        // New incoming connection, just store it
        m_peer[fromAddress] = itHeader;
      }
      else {
        m_peer[fromAddress] = itHeader;
      }
      m_peer_header = itHeader;
      // std::cout<<"Server received INT header:";
      // m_peer_header.Print(std::cout);
    }
    p->AddHeader(temp);
  }
}
                          ...
                          Ptr<Ipv4Interface> incomingInterface)
{
  ...
  if(m_int_das) {
    ProcessINT(tcpHeader, fromAddress, toAddress, packet);
  }
...
void
TcpSocketBase::SetPeerINT(IntHeader peerINT) {
  m_peer_header = peerINT;
}
                              ...
                              const Address& fromAddress, const Address& toAddress)
{
  ...
  newSock->SetAttribute("EnableINT", BooleanValue(m_int_das));
  newSock->SetAttribute("INT_CSMode", BooleanValue(m_das_mode));
  NS_LOG_LOGIC ("Cloned a TcpSocketBase " << newSock);
  if(m_int_das && m_das_mode) {
     newSock->SetPeerINT(m_peer_header);
  }
...
void
TcpSocketBase::AddOptionInt(TcpHeader& header, Ptr<Packet> p) {
  if(m_use_edf) {
    Ptr<IntTcpOption> intOption = CreateObject<IntTcpOption>();
    intOption->SetDeadline(m_deadline);
    header.AppendOption(intOption);
    return;
  }
  // INT related
  if( m_das_mode == 0) {
    // Client mode
    // <<"Appending INT client"<<std::endl;
    Ptr<IntTcpOption> intOption = CreateObject<IntTcpOption>();
    m_c_header.setTime(Simulator::Now().GetMicroSeconds());
    m_c_header.time_stamp = Simulator::Now().GetMicroSeconds();
    uint64_t sending_time = m_c_header.time_stamp - start_time;
    m_c_header.deadline = m_deadline > sending_time? m_deadline - sending_time: 0;
    if(m_c_header.mode == 2)  m_c_header.SetUrgency();
    intOption->SetDeadline(m_c_header.deadline);
    header.AppendOption(intOption);
    p->AddHeader(m_c_header);
  }
  else{
    // std::cout<<"Appending INT server"<<std::endl;
    Ptr<IntTcpOption> intOption = CreateObject<IntTcpOption>();
    m_peer_header.setTime(Simulator::Now().GetMicroSeconds());
    m_peer_header.mode = 3;
    intOption->SetDeadline(m_peer_header.deadline);
    header.AppendOption(intOption);
    p->AddHeader(m_peer_header);
  }
}
