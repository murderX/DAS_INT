...
protected:
  ...
  void AddOptionInt( TcpHeader& header, Ptr<Packet> p);

  void ProcessINT(TcpHeader tcpHeader, Address fromAddress, Address toAddres, Ptr<Packet> p);

  void SetPeerINT(IntHeader peerINT);
...
protected:
  ...
  // Param for INT
  bool m_int_das;
  bool m_das_mode; // 0 for client, 1 for server
  bool m_use_edf;
  uint32_t m_deadline;
  IntHeader m_c_header;
  std::map<Address, IntHeader> m_peer;
  IntHeader m_peer_header;
  uint64_t start_time;
  double m_alpha;
};
